...
class BKIT_MVP_ClosedDays_Admin {
    ...
    add_submenu_page(
        'bookingkit',
        __('Closed Days (Calendar)', 'bookingkit-mvp'),
        __('Closed Days (Calendar)', 'bookingkit-mvp'),
        'manage_options',
        'bookingkit_closed_days',
        [__CLASS__, 'render_admin_page']
    );
    }


    /* ====== Admin-Kalender (nur einmal, nur der neue) ====== */
    public static function render_admin_page(){
        ...
        wp_localize_script('bookingkit-admin','BKIT_MVP_ADMIN',[ 'nonce' => wp_create_nonce('bkit_mvp_admin') ]);
        ...
        for ($day = 1; $day <= $daysInMonth; $day++) {
            ...
            $cells[] = ['day'=>$day,'date'=>$date,'state'=>$state,'past'=>$past,'reason'=>$reason];
        }

        echo '<div class="wrap"><h1>'.esc_html__('Closed Days (Calendar)','bookingkit-mvp').'</h1>';
        ...
        foreach ($cells as $c) {
            ...
            printf('<button class="%s" data-date="%s"%s type="button"><span class="num">%d</span></button>',
                esc_attr($classes), esc_attr($c['date']), $reasonAttr, (int)$c['day']);
        }
        echo '</div>'; // grid
